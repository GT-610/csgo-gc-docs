...
GC 启动时会检查库存格式，并在控制台报告问题。这些问题不影响你正常游玩和使用库存，但还是建议按日志提示调整数据。
...
示例价格表包含商店布局、商品链接、价格、折扣价格和本地 GC 响应所需的元数据。价格由本地 GC 在运行时下发，修改文件后请重启游戏。

商店购买完全在本地模拟，不会扣除 Steam 钱包余额，也不会把物品加入 Steam 官方库存。本地 GC 同时只接受一笔结账交易；购买成功后会把新物品直接发送到本地 SOCache，因此无需重启游戏即可在库存中看到物品。
...
示例文件来自 2023 年 5 月 5 日的公开多币种快照。其中包含变革武器箱、Anubis 和 Paris 2023 等后期 CS:GO 商店条目，但不是 CS:GO 于 2023 年 9 月 27 日下架前 Valve 的原始价格表。
